...
## 23. [vue3的调度系统](https://juejin.cn/post/7055848604933554207)
...
1. 组件复用时，保证每个实例有自己的独立数据
当你使用组件时，每次使用组件都会创建一个新的实例。如果 data 是一个函数，那么每个组件实例调用这个函数，都会返回一个全新的对象副本，从而避免多个组件实例共享同一份数据
2. 如果 data 是对象，会导致所有实例共享同一个数据对象
...
### 25.1 vue2的watch实现机制
1. 响应式原理
Vue 2 使用 Object.defineProperty() 对对象的属性进行劫持，在 getter 中收集依赖，在 setter 中触发更新
2. watch实现过程
- 在组件初始化时，initWatch() 会遍历 watch 配置
- 每个侦听器会创建一个内部的 Watcher 实例
- Watcher 中的 getter 会主动访问被监听的属性，从而触发该属性的 getter，完成依赖收集
- 当数据变化时，对应属性的 setter 被调用，通知对应的 watcher，执行回调函数
### 25.2 vue3的watch实现机制
1. 响应式原理
Vue 3 重写了响应式系统，使用了 ES6 的 Proxy 替代 defineProperty，并引入了 ReactiveEffect 机制（也称为响应式副作用）
2. watch实现过程
- 调用 watch(source, callback) 时，Vue 会创建一个 ReactiveEffect
- Vue 在内部运行这个 effect，并追踪 source 所访问的响应式对象
- Vue 通过“依赖收集”记录哪些响应式属性被读取
- 当这些属性变化时，自动触发 ReactiveEffect 重新执行，并调用回调函数
### Vue 2 和 Vue 3 watch 实现的关键区别
| 对比项         | Vue 2                       | Vue 3                      |
| ----------- | --------------------------- | -------------------------- |
